...
const makeCI = (appId: number, spec: any) => {
...
export const mint = async (options: MintOptions) => {
  ...
  const mintR = await ci.mint(
    ...
    stringToUint8Array(options.symbol, 8),
    options.decimals,
    options.totalSupply
  );
  if (options.debug) {
...
export const arc200_balanceOf = async (options: Arc200BalanceOfOptions) => {
  const ci = makeCI(options.appId, HelloWorldSpec);
